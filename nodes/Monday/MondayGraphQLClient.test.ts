...
describe('MondayGraphQLClient', () => {
	...
	beforeEach(() => {
		mockContext = {
			...
			helpers: {
				...
				waitForExecutionFunction: vi.fn(),
			},
		};
		// baseRetryDelayMs = 1 so retry tests don't sleep for real
		client = new MondayGraphQLClient(mockContext, 'mondayApi', '2024-10', 1);
	});
	...
	describe('execute', () => {
		...
		it('should include variables in request', async () => {
			...
			await client.execute('query($boardId: Int!) { boards(ids: [$boardId]) { id } }', 0, variables);
				...
				errors: [
					{
						...
						extensions: {
							...
							status_code: 422,
							error_data: [
								{ itemId: null, columnIds: ['numeric_mm5bccxb'], message: "'vnumbers' must be at least [5]" },
	...
	describe('API version', () => {
		it('should default to the pinned MONDAY_API_VERSION constant', async () => {
			const { MONDAY_API_VERSION } = await import('./constants');
		...
		it('should use custom API version if provided', async () => {
			const customClient = new MondayGraphQLClient(mockContext, 'mondayApi', '2024-01');
			mockContext.helpers.httpRequestWithAuthentication.mockResolvedValue({
				data: { me: { id: '123' } },
				errors: undefined,
			});
