...
// Type-only imports from the official monday SDK (@mondaydotcomorg/api, devDependency).
// Erased at compile time — the SDK's runtime (graphql-request, zod, ...) must NOT be
// bundled: n8n verification requires zero runtime deps and all HTTP must go through
...
export function parseRateLimitHeader(value?: string): IDataObject | undefined {
	...
	for (const entry of value.split(',')) {
		...
		for (const rawParam of rawParams) {
			...
			const rawValue = rawParam.slice(separatorIndex + 1).replace(/"/g, '').trim();
...
export class MondayGraphQLClient {
	...
	private async executeInternal(
	...
	): Promise<{ data: IDataObject; requestInfo?: MondayRequestInfo; errors?: MondayGraphQLError[] }> {
		...
		throw lastError || new NodeApiError(this.context.getNode(), {
	...
	async uploadFile(
		...
		const response = (typeof raw === 'string' ? JSON.parse(raw) : raw) as MondayGraphQLResponse;
		if (response.errors && response.errors.length > 0) {
	...
	private async makeRequest(
		...
		try {
			const raw = await this.context.helpers.httpRequestWithAuthentication.call(this.context, this.credentialName, {
				method: 'POST',
				url: MONDAY_API_URL,
				body,
				json: true,
			...
			if (error instanceof Error) {
				...
				const nodeApiError = new NodeApiError(this.context.getNode(), error as unknown as JsonObject, {
	...
	private mapError(error: MondayGraphQLError, response: MondayGraphQLResponse): NodeApiError {
		...
		switch (errorCode) {
			...
			case MondayErrorCode.NotAuthenticated:
				...
				description = 'Invalid API token or insufficient permissions. Check your monday.com API token.';
				break;
