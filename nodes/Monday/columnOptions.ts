...
export async function getAggregateNumericColumns(
	...
	return toOptions(
		(await fetchBoardColumnRows(this)).filter((column) =>
			NUMERIC_AGGREGATE_TYPES.has(column.type),
		),
	);
}
...
export async function getAggregateMinMaxColumns(
	...
	return toOptions(
		(await fetchBoardColumnRows(this)).filter((column) =>
			MIN_MAX_AGGREGATE_TYPES.has(column.type),
		),
	);
}
 ...
 */
export async function getColumnLabels(this: ILoadOptionsFunctions): Promise<INodePropertyOptions[]> {
