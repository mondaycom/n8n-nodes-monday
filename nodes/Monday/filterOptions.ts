...
import { FOLDER_ROOT_PARENT, formatFolderLabel } from './folders';
import { MondayGraphQLClient } from './MondayGraphQLClient';

/**
 * loadOptions used by the Get Many Boards filters. multiOptions dropdowns
 * come with a client-side search box in the n8n UI, and expression mode is
 * the escape hatch for anything not in the loaded window (accepts an array
 * or a comma-separated string — see normalizeIdList).
 */
...
/** Folder dropdown for Project: Create (workspace under createProjectOptions). */
export async function getProjectWorkspaceFolders(
	this: ILoadOptionsFunctions,
): Promise<INodePropertyOptions[]> {
	return await loadWorkspaceFolders(this, 'createProjectOptions.workspaceId');
}

/** Folder dropdown for Form: Create (workspace is the top-level formWorkspaceId). */
export async function getFormWorkspaceFolders(
	this: ILoadOptionsFunctions,
): Promise<INodePropertyOptions[]> {
	return await loadWorkspaceFolders(this, 'formWorkspaceId');
}

/** Folder dropdown for Article: Create (top-level articleWorkspaceId). */
export async function getArticleWorkspaceFolders(
	this: ILoadOptionsFunctions,
): Promise<INodePropertyOptions[]> {
	return await loadWorkspaceFolders(this, 'articleWorkspaceId');
}

/** Folder dropdown for Doc: Create (top-level docWorkspaceId). */
export async function getDocWorkspaceFolders(
	this: ILoadOptionsFunctions,
): Promise<INodePropertyOptions[]> {
	return await loadWorkspaceFolders(this, 'docWorkspaceId');
}

/** Folder dropdown for Doc: Import From HTML (top-level docImportWorkspaceId). */
export async function getDocImportWorkspaceFolders(
	this: ILoadOptionsFunctions,
): Promise<INodePropertyOptions[]> {
	return await loadWorkspaceFolders(this, 'docImportWorkspaceId');
}

/**
 * Folder-resource variant of the workspace-folder loader. Unlike the
 * board-op loaders above, an unset workspace falls back to the account's
 * Main workspace via `workspace_ids: [null]` (verified live) — the
 * workspaces query never lists Main, so null is its only handle. Labels are
 * parent-aware ("Parent / Sub") because the API list is flat.
 */
async function loadFolderResourceFolders(
	context: ILoadOptionsFunctions,
	workspaceParamPath: string,
): Promise<INodePropertyOptions[]> {
	const workspaceId = context.getCurrentNodeParameter(workspaceParamPath, {
	...
	const client = new MondayGraphQLClient(context);
	interface FolderRow extends NamedRow {
	...
	for (let page = 1; page <= MAX_FOLDER_PAGES; page++) {
		const data = await client.execute(
			'query ($workspaceIds: [ID], $limit: Int!, $page: Int!) { folders(workspace_ids: $workspaceIds, limit: $limit, page: $page) { id name parent { id name } } }',
			0,
			{ workspaceIds: [workspaceId || null], limit: FOLDERS_PAGE_SIZE, page },
		);
		const pageRows = (data.folders ?? []) as FolderRow[];
		rows.push(...pageRows);
		if (pageRows.length < FOLDERS_PAGE_SIZE) break;
	}

	return rows
		.map((row) => ({ name: formatFolderLabel(row), value: row.id }))
		.sort((a, b) => a.name.localeCompare(b.name));
}

/** Folder picker for Folder: Update / Delete (top-level folderWorkspaceId). */
export async function getFolderList(this: ILoadOptionsFunctions): Promise<INodePropertyOptions[]> {
	return await loadFolderResourceFolders(this, 'folderWorkspaceId');
}

/** Parent-folder picker for Folder: Create (workspace under createFolderOptions). */
export async function getCreateFolderParentList(
	this: ILoadOptionsFunctions,
): Promise<INodePropertyOptions[]> {
	return await loadFolderResourceFolders(this, 'createFolderOptions.workspaceId');
}

/**
 * Destination-folder picker for Workspace Object: Move (workspace under
 * moveDestination). Falls back to the Main workspace's folders when no
 * destination workspace is selected.
 */
export async function getMoveDestinationFolderList(
	this: ILoadOptionsFunctions,
): Promise<INodePropertyOptions[]> {
	return await loadFolderResourceFolders(this, 'moveDestination.workspaceId');
}

/**
 * Parent-folder picker for Folder: Update — same list as getFolderList plus
 * a "workspace root" sentinel (to un-nest), minus the folder being updated
 * (a folder can't be its own parent).
 */
export async function getUpdateFolderParentList(
	this: ILoadOptionsFunctions,
): Promise<INodePropertyOptions[]> {
	const folders = await loadFolderResourceFolders(this, 'folderWorkspaceId');
	const selfId = this.getCurrentNodeParameter('folderId') as string | undefined;
	return [
		{ name: 'No Parent (Workspace Root)', value: FOLDER_ROOT_PARENT },
		...folders.filter((option) => option.value !== selfId),
	];
}
